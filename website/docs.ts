 ...
 * canonical English source to the single English route tree.
 */
...
/** Fields every manifest entry projects, besides the uniform projection fields. */
...
/** Build a page, attaching the uniform single-locale `contentLocale`. */
function page(fields: PageFields): DocsPage {
  return { ...fields, contentLocale: 'en-US' }
}
...
function tutorialPages(rows: readonly (readonly [string, string])[]): DocsPage[] {
  return rows.map(([file, label], order) => page({
...
const cordisTutorial = tutorialPages([
  ['index.md', 'Overview'],
...
const subsystemsReference = subsystemGroups.flatMap(([section, files]) =>
  files.map(([file, label], order) => page({
...
const reference = [
  ...
  ] as const).map(([source, route, label, order]) => page({
    source,
    route,
    label,
    sidebar: 'reference',
  ...
  ] as const).map(([source, route, label, outline], order) => page({
    source,
    route,
    label,
    sidebar: 'reference',
  ...
  ...(([
    ...
    ['service.md', 'Service'],
  ] as const).map(([file, label], order) => page({
  ...
  // `inherited.md` has no translated sibling, so it projects its canonical English source directly.
  page({
 ...
 * @throws When no group is declared for the label. Ranking by list membership
 *   alone would sort an undeclared group silently ahead of every declared one.
 */
