...
- {#c-legacy-credentials} Legacy pi-ai account references MUST migrate atomically to credential records before authentication reads them; an existing record MUST win, invalid legacy data MUST remain intact with a secret-free diagnostic, and removing the obsolete reference MUST prevent resurrection after logout.
