...
- {#c-session-cost} The system MUST estimate a session's USD cost from every settled usage's durable actual provider/model attribution and that routed OpenRouter model's price, sum routed OpenRouter models, and report an explicitly unpriceable cost rather than a zero when attribution, route eligibility, or a price is unavailable.
...
- {#c-cache} Repeated reads MUST be served from a bounded cache with a deployment-configurable lifetime, MUST de-duplicate concurrent reads, and MUST invalidate key readings when the non-secret credential identity changes; failed reads MUST not delay recovery.
