...
The same application distribution also includes the shared PendingInteractions registry, approval and question carriers, plan-review narrowing and Remote request registration. Retain their canonical pending-map augmentation and class identities in the common declaration graph; native applications must not install separate independently flattened copies. Qualification must install the exact archives, compile discriminated pending reads and Remote registration against the existing Session/Remote types, and exercise answers, cancellation and disposal through the installed runtime and iOS bundler. These packaging checks do not establish actual Remote transport or device answering.
...
## Portable pending interactions

Expose the existing Session-scoped pending-interaction registry through an ordinary ESM entry without React, Slots, browser globals or Host implementations. The browser adapter and native composition use that same owner and one canonical declaration-merged interaction map. Keep each domain registration owned by its caller's Cordis fiber. Preserve exact request objects, deterministic precedence, unchanged snapshot identity, contained subscriber failures and idempotent per-request removal. Domain teardown first withdraws all visible values, then delegates every outstanding request and awaits completion even when one delegate fails. A publisher cannot accept new requests after its domain has been released.
...
A failed pending publication must settle and observe its carrier before propagating the failure, including an already aborted request, so no listener or rejected promise survives the failed handoff. Qualify known and absent Session scopes, answers, cancellation, already-aborted and later-aborted delivery, plugin disposal with a blocked next listener, mixed-domain precedence and publication rejection through the shared registry. Built-artifact probes must refuse Node/browser dependencies. Portable pending carriers and completion barriers use the baseline Promise constructor and must execute when `Promise.withResolvers` is absent, as on the installed iPhone Hermes runtime. Do not add a global Promise polyfill or infer runtime support from bytecode compilation or a Node-backed VM. Preserve one-shot answers, cancellation and disposal in that restricted runtime. Existing browser composition remains covered; application packaging, actual Remote transport and physical-device answering are separate required acceptance gates.

## Observed prompt admission

The Client Session owner exposes a renderer-independent observer for one request identity and one Session binding. Report observed admission only when the exact request id appears in that binding's authoritative queue or a durable user-message event. Local submission echoes, identical text, another Session and an empty or incomplete history window cannot establish admission or rejection. A positive observation remains valid after the queue entry disappears or the event window is replaced. Follow existing queue and contiguous-history publications, process append/prepend deltas without repeatedly scanning unchanged history, and release both subscriptions on observation or disposal. The observer issues no mutation or extra history request and never retries a prompt.

Expose the same implementation through the installable application Client. Qualify history and queue arrival, replacement and revision gaps, unrelated messages, late result races and cleanup. The native form retains its original request identity across an uncertain reply and clears only the matching submitted draft when the shared observer proves acceptance. Absence remains unknown; safe replay after an absent observation requires a separate Host guarantee.
