...
The pinned native action provider uses `forge-intellect-action-tools/v2`. Adapter capabilities and checkpoint/close validation must name that same version. The composition fixture exercises v2, and an explicit native executable can replace the fixture to verify checkpoint and restart behavior against the packaged contract.
