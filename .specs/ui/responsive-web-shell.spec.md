...
- {#c-document-scroll} Below the phone breakpoint, the Chat transcript MUST use the main document as its vertical scroll owner so mobile browsers can retract their chrome, while preserving bottom-follow, semantic position restoration, prepend anchoring, the back-to-bottom action, sticky composer reachability, and a solid floating session header that hides during downward scrolling and returns during upward scrolling.
...
- {#c-touch-interaction} The page MUST prevent boundary overscroll from invoking browser refresh or navigation, and hover-only previews MUST remain closed when the primary input cannot hover.
