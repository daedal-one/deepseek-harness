...
Model-backed policy review adds latency to unmatched operations in policy-reviewed mode. A disposable local execution world can remove that repeated decision from file and process operations when configured container controls prevent those operations from reaching host files, credentials, processes, sockets, or external networks. The container image, Docker Engine, host kernel, and Harness control plane remain trusted.
...
- {#c-world} One runtime owner MUST provide one private host directory below `/tmp`, mounted as the only persistent writable volume in the container path namespace. The filesystem and subprocess service definitions MUST expose an opaque execution-world identity; the container providers MUST publish the runtime owner's identity, and a composition validator MUST fail load when the two mounted providers differ. The providers MUST map every configured Session cwd to the runtime's canonical `/workspace` path without exposing the host backing path.
...
- {#c-lifecycle} The initial implementation MUST own exactly one execution world per DSH process. It MUST create a random owner-only backing directory, enforce configured memory, CPU, PID, output, and lifetime limits, and own setup rollback, process quiescence, container removal, and backing-directory cleanup. Per-Session routing requires a separate accepted task.
- {#c-data} Host workspace import and sandbox output export MUST be explicit bounded operations that reject symlinks and special files. The initial implementation MAY start with an empty volume and omit import/export while documenting that limitation.
- {#c-evidence} Focused unit and Loader-composition tests MUST prove cross-capability visibility, lifecycle cleanup, fail-closed setup, absence of tool-policy decisions for contained operations, and retained independent authorization for one external operation. Mandatory Docker Engine coverage for every supported root mode MUST prove mount, host-sentinel, network, environment, namespace, device, resource-limit, and cleanup behavior before the composition can disable review. Real-engine evidence MUST report cold world creation and repeated warm no-op operation latency separately from model and network latency before the composition is presented as a policy-reviewed speed replacement.
:::
