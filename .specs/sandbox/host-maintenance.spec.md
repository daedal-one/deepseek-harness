...
- {#c-world} Disabling the host file sandbox MUST NOT move a container or remote execution world onto the host. Unsupported compositions MUST reject the option with an actionable error before boot. Host maintenance MUST use a separately launched host-backed profile.
...
- {#c-daedal-handoff} Only the Daedal presets MUST receive execution-environment guidance and a model-facing host handoff action. Container agents MUST request that action for host maintenance instead of probing host paths, processes, or unavailable tools. The action MUST require an explicit human confirmation of the destination and complete transferred task, preserve the source session's execution environment, and create a separate session in a configured host-backed profile. Missing configuration, delegated callers, rejection, unavailable interaction, and cancellation before dispatch MUST NOT start host work. A transport failure after dispatch MUST report uncertain acceptance without automatically retrying.
