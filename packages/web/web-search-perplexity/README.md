...
This is an **implementation** package: it registers a provider into `ctx.web`, it does not own the key and it does not register a model-facing tool. It is a function/namespace plugin (`inject: ['web']`). The OpenAI-compatible wire format is a provider-private detail — it does **not** make this provider depend on `ctx.llm`.
