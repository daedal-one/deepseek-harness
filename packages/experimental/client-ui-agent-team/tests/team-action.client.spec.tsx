...
function props(actions: TeamActionInjected, sessionId: SessionId = SESSION): TeamActionProps {
  return {
    sessionId,
    ...actions,
    t: makeTranslate(copy, commonCopy),
...
describe('TeamAction', () => {
  ...
  it('creates a task from normalized blocker and write-scope lists', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: ' Details ' } })
    fireEvent.change(screen.getByPlaceholderText(/Blocking task ids/u), { target: { value: 'task-1, task-1' } })
  ...
  it('assigns, edits, completes, reopens, and deletes with contiguous CAS revisions', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Blocking task ids/u), { target: { value: 'task-0' } })
  ...
  it('keeps reload failures visible after task and dependency conflicts', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(copy.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
