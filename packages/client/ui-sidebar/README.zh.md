...
侧边栏外壳插件：负责实时产品标记与名称、New Session 操作、布局持有的折叠控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。产品身份来自 [ui-branding](../ui-branding/README.md)；展开的侧边栏显示两者，收起后的 56px 轨道保留标记。[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠到布局持有的轨道仍属于本地呈现行为。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md)。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions`、`useWorkspaces` 和 `useBranding` 钩子、已声明的 `sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
