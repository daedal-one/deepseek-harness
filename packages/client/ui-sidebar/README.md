...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots; the collapsed rail renders the same mark slot. Without occupants, the shell uses the Daedal companion mark and a localized Daedal Harness title. A complete build stacks a code badge below the label as `version[-commit][-dirty]`, using `DSH_CLIENT_VERSION`, the optional 7-character `DSH_CLIENT_COMMIT_HASH`, and `DSH_CLIENT_GIT_DIRTY=true`; missing version metadata omits the badge. New Session targets the explicit Workspace used by a scoped action, otherwise the current Session's Workspace, otherwise the most recently active Workspace; when none exists it clears into the blank New Session page.

The expanded logo and title open the fork overview on every activation, including the first. The overview stays closed on startup and reload, credits DeepSeek upstream, and summarizes implemented fork additions. Escape, the close button, and the backdrop dismiss it; focus returns to the brand control. The dialog keeps background controls inert, contains keyboard focus, and scrolls within phone-sized viewports. Its visibility is component-local and does not write settings.
