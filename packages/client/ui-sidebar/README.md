...
Sidebar shell plugin: the live product mark and name, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. The product identity comes from [ui-branding](../ui-branding/README.md); the expanded sidebar shows both values and the collapsed 56px rail keeps the mark. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions`, `useWorkspaces`, and `useBranding` hooks, the declared `sidebar.workspaces` and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
