...
const t: OpenInAppActionProps['t'] = makeTranslate(copy)
...
describe('OpenInAppAction visibility', () => {
  ...
  it('shows the remembered choice, falling back to the first available app when it is gone', () => {
    ...
    expect(screen.getByRole('button', { name: copy['open.title'].replace('{app}', 'Cursor') })).toBeDefined()
    cleanup()

    render(<OpenInAppAction {...bench({ apps: ['finder', 'cursor'], choice: 'vscode', cwd: '/w' }).props} />)
    expect(screen.getByRole('button', { name: copy['open.title'].replace('{app}', copy['app.finder']) })).toBeDefined()
  })
})
...
describe('OpenInAppAction launching', () => {
  it('launches without painting the busy dress when the launch settles quickly', async () => {
    ...
    const main = screen.getByRole('button', { name: copy['open.title'].replace('{app}', copy['app.finder']) })
  ...
  it('shows the error state and decays back to idle after a fast failure', async () => {
    ...
    fireEvent.click(main)
    await waitFor(() => {
      expect(screen.getByRole('button', { name: copy['open.error'] })).toBeDefined()
    })
  ...
  it('shows the product tooltip on hover instead of a native title', async () => {
    ...
    expect(await screen.findByText(copy['open.tooltip'])).toBeDefined()
    fireEvent.mouseLeave(main)
    await waitFor(() => {
      expect(screen.queryByText(copy['open.tooltip'])).toBeNull()
    })
  })
  ...
  it('opens the menu from the chevron, launches and persists a picked app', async () => {
    ...
    render(<OpenInAppAction {...b.props} />)
    fireEvent.click(screen.getByRole('button', { name: copy['menu.toggle'] }))
  ...
  it('ignores a menu pick while a launch is in flight', async () => {
    ...
    render(<OpenInAppAction {...b.props} />)
    fireEvent.click(screen.getByRole('button', { name: copy['open.title'].replace('{app}', copy['app.finder']) }))
    expect(b.launch).toHaveBeenCalledTimes(1)
    fireEvent.click(screen.getByRole('button', { name: copy['menu.toggle'] }))
  ...
  it('clears a pending error decay when a retry starts', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: copy['open.title'].replace('{app}', copy['app.finder']) }))
    await act(async () => {})
    fireEvent.click(screen.getByRole('button', { name: copy['open.error'] }))
  ...
  it('closes an open menu on Escape without launching', async () => {
    ...
    render(<OpenInAppAction {...b.props} />)
    fireEvent.click(screen.getByRole('button', { name: copy['menu.toggle'] }))
    await screen.findByText(copy['app.terminal'])
    fireEvent.keyDown(document, { key: 'Escape' })
    await waitFor(() => {
      expect(screen.queryByText(copy['app.terminal'])).toBeNull()
    })
