...
async function bench(preference?: string) {
  ...
  ctx.effect(() => locale.addLanguage({ id: 'es', label: 'Español', fallback: 'en' }))
  return { ctx, locale }
}
...
describe('document language', () => {
  ...
  it('states the resolved locale at activation, not the value the markup shipped', async () => {
    // A Spanish browser resolves the registered language even though the markup said en.
  ...
  it('follows an explicit Host preference that overrides browser detection', async () => {
    // Stored preference wins over the Spanish browser pinned above.
