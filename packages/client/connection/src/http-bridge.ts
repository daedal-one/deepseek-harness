 ...
 * @param authorization - optional device revocation lifetime; revocation terminates this carrier.
 */
export async function bridge(
  ...
  authorization?: AbortSignal,
): Promise<void> {
  if (authorization?.aborted === true) {
    res.writeHead(401)
  ...
  const revoke = (): void => { abort.abort(authorization?.reason); res.destroy() }
  authorization?.addEventListener('abort', revoke, { once: true })
  try {
    ...
    res.end()
    if (requestUnread) req.destroy()
  } finally { authorization?.removeEventListener('abort', revoke) }
}
