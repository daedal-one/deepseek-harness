...
`POST /api/connection/devices/claim` accepts `{ hostId, challenge, label }` without an existing credential, still behind the Host/Origin fence. This exact route has a fixed 2 KiB request limit; labels contain 1–80 characters after trimming. Challenges contain 256 random bits, expire by elapsed time, remain in memory and are consumed before a grant is persisted. Restart or plugin reload invalidates outstanding challenges. A failed or lost claim requires a fresh enrollment; it is never retried automatically. QR presentation and native secure storage belong to the application. Permanent credentials must never enter URLs or QR codes.
