...
The `@deepseek-ai/dsh-client-connection/client/portable` entry provides `createConnection` and `createConnectionRpc` as ordinary ESM without the Web module loader. Each connection receives an explicit RPC transport, local-Host hint, recovery configuration and optional network observer. Native callers supply their Host authority, authenticated Fetch and stream adapters, and correlation-id generator. The portable entry does not read page, fetch or crypto globals. The Web plugin supplies those inputs from its browser environment and uses the same connection implementation.
