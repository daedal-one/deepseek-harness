...
describe('FilesBody', () => {
  it('says so when the session has no workspace directory, and asks for nothing', () => {
    ...
    expect(view.container.querySelector('[data-files-state="no-workspace"]')?.textContent).toBe(copy.noWorkspace)
  ...
  it('a file click opens its session-scoped file: address through the owner; an other entry offers no button', async () => {
    ...
    expect(other.querySelector('[aria-disabled="true"]')?.getAttribute('title')).toBe(copy['entry.other'])
  })
  ...
  it('marks a cut listing and an empty one', async () => {
    ...
    expect(view.container.querySelector('[data-files-row="truncated"]')?.textContent).toBe(copy.truncated)
    ...
    expect(view.container.querySelector('[data-files-row="empty"]')?.textContent).toBe(copy.empty)
  })
  ...
  it('shows a failed level under its directory with the failure code', async () => {
    ...
    expect(failed?.textContent).toBe(copy['error.notFound'])
  })
  ...
  it('reload resets every level and lists the expanded ones again', async () => {
    ...
    expect(view.container.querySelector('[data-files-reload]')?.getAttribute('aria-label')).toBe(copy.reload)
  })
