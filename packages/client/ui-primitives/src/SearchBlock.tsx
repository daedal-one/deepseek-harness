 ...
 * card holds. The unit — `match · K files` for grep, `paths` for glob — trails
...
function summaryText(props: SearchBlockProps, shown: number, truncated: boolean, total: number): string {
  ...
  if (props.kind === 'paths') return `${count} ${shown === 1 ? 'path' : 'paths'}`
  const files = props.files.length
  return `${count} ${shown === 1 ? 'match' : 'matches'} · ${files} ${files === 1 ? 'file' : 'files'}`
}
          ...
          <div className={css.body}>
            ...
            {hidden > 0 && (
              <button
                ...
                aria-label={expanded ? 'Collapse results' : `Show ${hidden} more results`}
                onClick={onToggle}
              >
                {expanded ? 'Collapse' : `… ${hidden} more rows`}
              </button>
            )}
