...
The first batch covers the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and the OpenRouter search provider (`web-search-openrouter`).
