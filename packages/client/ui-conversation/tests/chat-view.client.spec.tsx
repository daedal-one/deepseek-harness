...
describe('ChatView', () => {
  ...
  it('the settled footer appends first-step ttft and turn output rate', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    // First-step ttft (1.2s) plus 100 tokens over 6.4s of request wall time.
    expect(view.getAllByText(/Ran for 19s/)).toHaveLength(1)
    expect(view.getAllByText(/TTFT 1\.2s/)).toHaveLength(1)
    expect(view.getAllByText(/16 tok\/s/)).toHaveLength(1)
  })
  ...
  it('renders /compact as one stateful disclosure from running through completion', () => {
    ...
    expect(view.queryByText('Facts to keep.')).toBeNull()
    fireEvent.click(row)
    expect(row.getAttribute('aria-expanded')).toBe('true')
    expect(row.querySelector('[data-compaction-disclosure="expanded"]')).not.toBeNull()
    expect(view.getByRole('heading', { name: 'Compacted summary' })).toBeTruthy()
  })
