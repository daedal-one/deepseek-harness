...
const WELCOME_NOTICE_COPY = {
  ...
  copy: { title: copy.welcomeTitle, body: copy.welcomeBody, continueLabel: copy.welcomeContinue },
}
...
describe('WelcomeNotice', () => {
  it('uses the exact owner copy in both GUI locales', () => {
    ...
    expect(copy.welcomeBody).toBe(WELCOME_NOTICE_COPY.copy.body)
  })

  it('renders one blocking modal action and focuses the title', async () => {
    const h = mount()
    const dialog = await screen.findByRole('dialog', { name: WELCOME_NOTICE_COPY.copy.title })
    for (const paragraph of WELCOME_NOTICE_COPY.copy.body.split('\n\n')) {
      expect(screen.getByText(paragraph, { exact: true })).toBeTruthy()
    }
    expect(dialog.querySelectorAll('p')).toHaveLength(2)
    expect(dialog.querySelectorAll('button')).toHaveLength(1)
    expect(screen.getByRole('button', { name: WELCOME_NOTICE_COPY.copy.continueLabel })).toBeTruthy()
    expect(document.activeElement).toBe(screen.getByRole('heading', { name: WELCOME_NOTICE_COPY.copy.title }))
  ...
  it('completes only after the acknowledgement write commits', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: WELCOME_NOTICE_COPY.copy.continueLabel }))
  ...
  it('keeps the sole action disabled while saving and reports a refused write', async () => {
    ...
    await screen.findByRole('dialog')
    const action = screen.getByRole<HTMLButtonElement>('button', { name: WELCOME_NOTICE_COPY.copy.continueLabel })
    ...
    expect((await screen.findByRole('alert')).textContent).toBe(copy.welcomeError)
    expect(h.complete).not.toHaveBeenCalled()
  })
})
