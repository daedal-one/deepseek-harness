...
import type { OpenRouterOnboardingDialogProps } from '../src/client/OpenRouterOnboardingDialog.tsx'
...
const OpenRouterProfileConfig = Schema.object({
...
type AttentionSnapshot = Parameters<Parameters<OpenRouterOnboardingDialogProps['useSessionPendingInteraction']>[0]>[0]
const noAttention: AttentionSnapshot = new Map()
const useSessionPendingInteraction: OpenRouterOnboardingDialogProps['useSessionPendingInteraction'] = selector => selector(noAttention)

function openRouterNamespace(apiKeyEnv: string | null): SettingsNamespaceView {
  const value = apiKeyEnv === null ? {} : { apiKeyEnv }
  return {
    ns: 'llm-pi-ai',
    schema: JSON.parse(JSON.stringify(Schema.object({ providers: Schema.dict(OpenRouterProfileConfig) }).toJSON())) as JsonValue,
    value: { providers: { openrouter: value } },
    base: { providers: { openrouter: value } },
    user: {},
    applies: 'live',
    secrets: [],
    revision: 0,
...
function harness(options: {
  ...
  const apiKeyEnv = options.apiKeyEnv === undefined ? 'OPENROUTER_API_KEY' : options.apiKeyEnv
  const mutate = vi.fn(() => Promise.resolve(remoteOk(openRouterNamespace(apiKeyEnv))))
  ...
  const face = {
    ...
    settings: {
      describe: () => Promise.resolve(remoteOk({
        ...
        hasDocument: false,
        namespaces: options.settingsNamespace === false ? [] : [openRouterNamespace(apiKeyEnv)],
      })),
      mutate,
    },
    credentials: {
  ...
  const props: OpenRouterOnboardingDialogProps = {
    stepId: 'openrouter',
    complete,
    openSection,
...
describe('OpenRouterOnboardingDialog', () => {
  it('renders when the shell root is absent', async () => {
    const h = harness()
    document.getElementById('root')!.remove()
    render(<OpenRouterOnboardingDialog {...h.props} />)
  ...
  it('cannot be dismissed implicitly and restores the previous inert state', async () => {
    ...
    appRoot.inert = true
    const view = render(<OpenRouterOnboardingDialog {...h.props} />)
