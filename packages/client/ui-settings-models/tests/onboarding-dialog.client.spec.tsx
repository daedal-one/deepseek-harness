...
/** First-run OpenRouter prompt behavior over the shared Models join. */
...
const OpenRouterProfile = Schema.object({
...
const PiAiConfig = Schema.object({ providers: Schema.dict(OpenRouterProfile) })

function openRouterNamespace(apiKeyEnv: string | null): SettingsNamespaceView {
  const value = { providers: { openrouter: apiKeyEnv === null ? {} : { apiKeyEnv } } }
  return {
    ns: 'llm-pi-ai',
...
function harness(options: {
  ...
  const mutate = vi.fn(() => Promise.resolve(ok(openRouterNamespace(apiKeyEnv))))
...
describe('OpenRouterOnboardingDialog', () => {
  ...
  it('does not block the product when OpenRouter setup is unavailable', async () => {
    for (const h of [
