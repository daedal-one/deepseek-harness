...
The **Spend** tab is a conversation view showing what the configured OpenRouter inference key has spent and what the current Session is costing. It projects the Host's `openrouterSpend.read` reading into two blocks: the key's spend rows (total, daily, weekly, monthly, and the remaining limit when set, with a free-tier marker) and the session estimate for the routed model. It renders an in-flight state, the settled reading, and one distinct stated failure per Host-reported reason. The view owns no timers and holds no credentials; it reads on open and on request.
...
- **Unpriceable sessions are explicit, not zero** — when any settled route is outside OpenRouter, lacks attribution or a catalog price, the session estimate shows the unpriceable wording rather than any amount.
...
The spec suite feeds the view its composed seats directly (store instance plus `makeTranslate`) and drives the plugin wiring through a Cordis bench with a scripted `remote.openrouterSpend` face, so neither the conversation view owner nor the wire is required. The keyless owner-local [`spend-view.expected.md`](tests/spend-view.expected.md) captures configured and remaining limits, unlimited limits, priced and unpriceable sessions, and a failure state. Fixture amounts keep a non-zero last digit so the no-fabricated-zero assertions stay exact.
