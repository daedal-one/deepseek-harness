...
describe('SpendView', () => {
  ...
  it('renders the key spend rows, the limit, and the session estimate for a priced model', () => {
    ...
    expect(screen.getByText(en['row.limit'])).toBeTruthy()
  ...
  it('marks a free-tier key and renders the no-limit wording when the limit is absent', () => {
    const store = createSpendStore().create()
    ...
    expect(screen.getByText(en['row.limit'])).toBeTruthy()
    expect(screen.getByText(en['money.none'])).toBeTruthy()
  })
  ...
  it.each([
    ...
    expect(screen.getByRole('alert', { name: en[key] })).toBeTruthy()
