...
describe('host frame routing', () => {
  it('adds/removes/flips sessions from host frames and keeps removed instances resident', async () => {
    ...
    manager.handleHostEnvelope({ rpcId: 'h4' as never, payload: { type: 'host/agent-error', sessionId: S1, message: 'blew up' } })
    expect(session.getSnapshot().lastAgentError).toBe('blew up')
...
describe('remaining branches', () => {
  ...
  it('routes stream/error and unknown frames to the documented drops, and dispatches to instantiated sessions', () => {
    ...
    manager.handleHostEnvelope({ rpcId: 'ha' as never, payload: { type: 'host/agent-error', sessionId: S2, message: 'no instances' } })
  })
