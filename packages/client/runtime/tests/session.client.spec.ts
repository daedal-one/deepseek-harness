...
describe('open', () => {
  ...
  it('stitches live frames arriving while history is pending, dropping the page overlap', async () => {
    ...
    const page = plainTurn(10, 0, 'morning', 'night')
    session.handleMuxEnvelope('r1' as never, { type: 'session/event', sessionId: SID, event: ev.turnStart(15, 1) })
    session.handleMuxEnvelope('r2' as never, { type: 'session/event', sessionId: SID, event: ev.user(16, 'interleaved') })
    gate.resolve(ok({
...
describe('live event path', () => {
  ...
  it('drops replayed frames at or below the window tail', async () => {
    ...
    session.handleMuxEnvelope('r' as never, { type: 'session/event', sessionId: SID, event: ev.user(3, 'replay') })
  ...
  it('publishes animation-frame Definitions once per frame and lets an immediate event supersede the pending frame', async () => {
    ...
    feed(ev.chunkText(8, 1, 'umulated'))
    ...
    feed(ev.chunkText(9, 1, 'complete'))
    feed(ev.assistant(10, 1, 'accumulated complete'))
    await Promise.resolve()
    expect(published).toEqual([
      [0, 1, 2, 3, 4, 5, 6, 7, 8],
...
describe('paging', () => {
  ...
  it('installs a page without interpreting business replacement metadata', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse([
      ev.compactSummary(80, 'out-of-window summary', 3, 40),
      ev.compactCheckpoint(81, 80, 3, 40),
      ev.user(82, 'new question after compaction'),
    ], true)
...
describe('prompt and cancel errors', () => {
  ...
  it('business failure lands in promptError with op=send; the phase stays engaging (retry, no hero bounce)', async () => {
    ...
    const result = await session.prompt([{ type: 'text', text: 'to fail' }], 'queue')
...
describe('remaining branches', () => {
  ...
  it('drops a stale doOpen whose history resolved successfully after resync superseded it', async () => {
    ...
    api.onHistory = () => stale.promise
    const opening = session.open()
    api.onHistory = () => histResponse(plainTurn(6, 1, 'new', 'generation'))
    const resynced = session.resync()
    stale.resolve(ok({
      events: entries(plainTurn(0, 0, 'old', 'generation')) as never[],
      hasMore: false,
...
describe('reference stability (the memo contract)', () => {
  it('keeps unchanged node references across an append and swaps the snapshot object', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse(plainTurn(0, 0, 'sta', 'ble'))
    await session.open()
    const before = session.getSnapshot()
    ...
    session.handleMuxEnvelope('r' as never, { type: 'session/event', sessionId: SID, event: ev.user(6, 'append') })
  ...
  it('keeps unrelated Session arrays and settled Chat Nodes stable across Event updates', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse(plainTurn(0, 0, 'base', 'foundation'))
    await session.open()
    const feed = (event: SessionEvent) => { session.handleMuxEnvelope('r' as never, { type: 'session/event', sessionId: SID, event }) }
    ...
    feed(ev.chunkText(10, 1, 'streaming independent of tools'))
    const after = session.getSnapshot()
    expect(after).not.toBe(before)
    ...
    feed(ev.assistant(12, 1, 'complete'))
