...
Assistant 计时检查器把本地观测到的首 token 到完成区间命名为 `Stream span`，不会宣称该区间是提供方生成时间。`Output rate` 用提供方上报的输出 token 数除以从步骤开始到完成的完整请求墙钟时间，其中包含 TTFT 与缓冲投递，因此本地接收的 chunk 突发不会产生一个被标为 `tok/s` 的传输排空速率。
