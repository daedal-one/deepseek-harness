...
外壳拥有浏览器标题投影。选中带有持久标题的会话时，它会渲染 `<session title> — <current product name>`，并响应后续会话标题或品牌修订；未选择会话或选中无标题会话时，会渲染当前产品名称；外壳卸载时恢复 app-shell 激活前的标题。品牌服务可用之前，加载页面使用 Host 投影的 HTML 标题。
