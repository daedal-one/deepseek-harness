...
The shell owns browser-title projection. With a selected session carrying a durable title, it renders `<session title> — <current product name>` and reacts to later session-title or branding revisions; no selection or a selected untitled session renders the current product name, and shell unmount restores the title that preceded app-shell activation. The loading page uses the Host-projected HTML title until the branding service is available.
