...
async function mountPlugin(
  ...
  await ctx.plugin(PiAi, {
    providers: { openrouter: {
      ...
      baseURL: process.env.OPENROUTER_BASE_URL ?? 'http://127.0.0.1:9',
      models: [{ id: 'apply-model' }, { id: 'dsagent-model' }],
    } },
  })
...
describe('dsh-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    ...
    try {
      harness.send({ jsonrpc: '2.0', id: 'init-1', method: 'initialize', params: { cwd: storageDir, provider: 'openrouter', model: 'apply-model' } })
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    ...
    vi.stubEnv('OPENROUTER_API_KEY', 'test-key')
    vi.stubEnv('OPENROUTER_BASE_URL', llmServer.url)
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { cwd: storageDir, provider: 'openrouter', model: 'dsagent-model' } })
  ...
  it('answers shutdown before exiting 0 exactly once, even against a racing second shutdown', async () => {
    ...
    try {
      ...
      harness.send({ jsonrpc: '2.0', id: 'after-exit', method: 'initialize', params: { cwd: storageDir, provider: 'openrouter', model: 'x' } })
      await settle()
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    ...
    try {
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'after-flush-failure', method: 'initialize', params: { cwd: storageDir, provider: 'openrouter', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
    } finally {
      await harness.dispose()
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    ...
    try {
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'probe-2', method: 'initialize', params: { cwd: storageDir, provider: 'openrouter', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
      expect(harness.exits()).toEqual([])
    } finally {
