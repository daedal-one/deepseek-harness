...
The optional boolean `dsh.profile.sandbox` is validated and carried in the loaded `Profile`. The CLI applies `false` as [host-maintenance defaults](../../../apps/cli/reference/README.md#host-maintenance); omission and `true` preserve the composition. The manifest is read at launch, independently of patch-file reload.
