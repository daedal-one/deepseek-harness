...
- `ctx.llm.registerProviderAuthenticator(provider: string, authenticator: LlmProviderAuthenticator): () => void` Register one provider-owned interactive authentication method. The registration owns login, status, and logout and is disposed with its calling fiber.
- `ctx.llm.providerAuthentication(provider: string): Promise<LlmProviderAuthStatus[]>` Read the detached account state of every registered interactive method for one provider.
- `ctx.llm.startProviderAuthentication(provider: string, method: LlmProviderAuthMethod): LlmAuthOperationSnapshot` Start login and return a bounded background-operation snapshot immediately.
- `ctx.llm.authenticationOperation(id: LlmAuthOperationId): LlmAuthOperationSnapshot` Read one background login, including a provider-issued device code when available.
- `ctx.llm.cancelProviderAuthentication(id: LlmAuthOperationId): Promise<LlmAuthOperationSnapshot>` Abort and drain one pending login.
- `ctx.llm.logoutProvider(provider: string, method: LlmProviderAuthMethod): Promise<void>` Abort and drain any matching login before removing the stored account, so late completion cannot restore it.
...
Provider authentication is an adapter extension point rather than a property of the model loop. One provider/method may have one pending login; terminal snapshots remain queryable in a bounded 64-operation history. Authenticator disposal aborts and drains its work before returning. The service carries account state and device authorization metadata, never tokens; the authenticator owns storage and refresh.
...
[`@deepseek-ai/dsh-llm-pi-ai`](../llm-pi-ai) dynamically resolves configured provider/model pairs through `@earendil-works/pi-ai` and is the shipped conversation adapter. It follows the `StreamChunk` conventions in `types.ts`: usage precedes finish and tool arguments remain raw strings. An adapter may throw or emit a failure finish internally; `LlmRuntime` exposes either as a terminal failure finish. See [the terminal-failure decision](../../../.agents/notes/implemented/architecture/2026-07-29-terminal-llm-stream-failures.md) for the service obligation.
