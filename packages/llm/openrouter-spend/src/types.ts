...
/** Session-level cost estimate for one session's effective model selection. */
export interface OpenRouterSessionSpend {
  /** Provider id of the session's effective model selection. */
  readonly provider: string
  /** Exact model id of the session's effective model selection. */
  readonly model: string
  /** Estimated session cost in USD; null when the route or a required price is unavailable; never a fabricated 0. */
...
export interface OpenRouterSpendSnapshot {
  ...
  /** Epoch milliseconds at which the key reading was fetched. */
