...
export class OpenRouterSpendService extends TypertRemoteService {
   ...
   * @param signal - carrier cancellation shared with the underlying OpenRouter reads.
  ...
  async read(request: OpenRouterSpendReadRequest, signal: AbortSignal): Promise<OpenRouterSpendReadResult> {
    ...
    const keyResult = await this.keyCache.read(() => readKeyUsage(options, signal))
    if (!keyResult.ok) return { ok: false, error: keyResult.error }
    const session = await this.sessionSpend(request.sessionId, options, signal)
   ...
   * @param signal - carrier cancellation for the catalog read.
  ...
  private async sessionSpend(
    ...
    const spend: OpenRouterSessionSpend = {
      ...
      costUsd: tokenUsage === undefined ? null : await this.priceSession(selected.model, tokenUsage, options, signal),
    }
    return spend
  }
  ...
  private async priceSession(
    ...
    const catalog = await this.catalogCache.read(() => readModels(options, signal))
