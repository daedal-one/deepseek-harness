...
OpenAI has two native routes. `openai` uses the OpenAI API and a key stored under `OPENAI_API_KEY`; `openai-codex` uses pi-ai's `openai-codex-responses` implementation and an OpenAI account authorized through the Models page's device flow. Codex access and refresh tokens are stored as one internal value in `ctx.credentials`, refreshed through atomic `modify`, and never enter settings or a client response. Logging out drains a pending login before deleting the account.
...
### Reasoning-dispatch compat switches and OpenRouter routing
...
`compat.openRouterRouting` carries provider-routing preferences that OpenRouter interprets when the model's route reaches it — the same `provider` request field that appending `:nitro` to a model id stands for. It takes pi-ai's `OpenRouterRouting` shape (`sort: throughput` chooses the fastest provider, `order`/`only`/`ignore` pin providers, `max_price` caps spend, and so on). Set it once on the route and every `openai-completions` model on it inherits the routing, so a deployment can adopt catalog model ids (e.g. `deepseek/deepseek-v4-flash`) with throughput routing instead of declaring a `:nitro` alias per wire id. A model-level routing object replaces the route object; omission inherits the route. Routing enums, non-empty lists, and non-negative thresholds are validated before the profile is stored, and the resolved object is sent verbatim as the request's `provider` field. Routing lives only on `openai-completions`: a route with no such model is refused.
...
Named API keys never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override, preserving fail-loud reference semantics. `Models` receives a credential-store bridge for provider-native OAuth accounts and refresh only. A route naming no key either resolves through that store, uses its catalog provider's ambient discovery, or remains configured-but-keyless and leaves the requirement to the protocol.
...
- **Interactive OAuth is implemented for OpenAI Codex only** — pi-ai providers that also offer an API key remain manageable through that key; another provider's OAuth method is not advertised until its prompt types have matching product controls.
