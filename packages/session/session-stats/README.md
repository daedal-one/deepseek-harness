...
- `throughputMs`/`throughputTokens` sum `step/start` → assembled message and the provider-reported output tokens, only over steps carrying both. Their ratio is an end-to-end output rate: full request time prevents buffered chunk delivery from being presented as model decode speed.
...
- **`tok/s` includes TTFT and in-step retries** — it is the observable end-to-end output rate, not provider decode throughput; the durable log has no provider-side generation clock.
