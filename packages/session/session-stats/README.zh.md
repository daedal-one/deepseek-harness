...
注册 `sessionStats` projection 单元的函数插件：从步边界、流式 chunk、工具配对与已组装的 assistant 消息折叠出全日志会话数字——轮/步计数以及 LLM、工具、首 token、输出速率墙钟时间——经 session-projection seam 对外提供（registry 快照、变更流，以及每一个 projection 载体：history 尾页、`session/projection` 推送帧、会话列表行）。客户端由此渲染分页与压缩都无法改变的全会话数字；参考消费方是 Web 聊天统计条，其窗口折叠以相同字段名充当无单元时的回退。
...
- `throughputMs`/`throughputTokens` 累加 `step/start` → 已组装消息的时长与提供方上报的输出 token，仅统计两者兼备的步。两者之比是端到端输出速率：完整请求时长可防止把缓冲后的 chunk 投递速度当作模型解码速度展示。
...
- **`tok/s` 包含 TTFT 与步内重试**——它是可观察的端到端输出速率，而非提供方解码吞吐；持久日志没有提供方侧的生成时钟。
