...
export const RELEASED_V0_EVENT_DISPOSITIONS: Readonly<Record<string, ReleasedV0PayloadDisposition>> = Object.freeze({
  ...
  'tool-policy/classifier-request': disposition(['turn', 'providerId', 'route', 'purpose', 'input', 'request'], ['callId']),
  'tool-policy/intent-context': disposition(['turn', 'requestSeq', 'userMessageSeq', 'providerId', 'allowedEffects', 'forbiddenEffects', 'summary']),
  'tool-policy/decision': disposition(['turn', 'callId', 'toolName', 'stage', 'policyDecision', 'effectiveDecision', 'providerId', 'risk', 'categories', 'reason']),
  'memory/extraction-request': disposition(['turn', 'sourceEventSeqs', 'route', 'system', 'messages', 'maxTokens'], ['sourceSessionFormatVersion']),
  'memory/extraction-result': disposition(['turn', 'blocks', 'finish', 'proposedIds'], ['usage', 'failure'], ['usage', 'failure']),
  'english-output/translation-request': disposition(['turn', 'step', 'target', 'translator', 'system', 'messages', 'maxTokens', 'blocks']),
  'english-output/translation-result': disposition(['turn', 'step', 'status', 'blockIndexes'], ['failure'], ['failure']),
  'web/openrouter-search-llm-request': disposition(['endpoint', 'body'], [], ['body']),
  ...
  'subagent/descriptor': disposition(
    ...
    ['label', 'agentProvider', 'agentModel', 'agentReasoningEffort', 'persona', 'toolFilter', 'principal'],
  ),
