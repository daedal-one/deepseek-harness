...
export class FakeApiClient {
  ...
  onForkTo: (payload: unknown) => Promise<RemoteResult<{ sessionId: SessionId }>> = () => Promise.reject(new Error('forkTo response is not programmed'))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        forkTo: payload => this.record('session.forkTo', payload, this.onForkTo(payload)),
