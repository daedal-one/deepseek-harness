...
Cancelled admission and forwarded listeners retain settlement observers on already-created Promises, even when cancellation precedes their delivery to the owner. Pre-existing cancellation takes precedence over an already fulfilled value; late failures cannot publish readiness, send a result RPC or escape as unhandled rejections. Observation does not prove that the underlying work stopped. See the [cancellation ownership decision](../../../.agents/notes/implemented/bug-fix/2026-09-23-remote-cancellation-observes-supplied-promises.md).
