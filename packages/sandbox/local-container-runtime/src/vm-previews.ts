...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  ctx.effect(() => ctx.connection.fetch.register({
    ...
    fetch: (request) => {
      ...
      if (!/^[a-zA-Z0-9_-]{1,128}$/u.test(sessionId) || !/^\d+$/u.test(portValue) || port < 1024 || port > 65535) return Promise.resolve(new Response('Invalid conversation or port', { status: 400 }))
      const workspace = ctx.conversationWorkspaces.runForSession(
        brandString<SessionId>(sessionId), () => ctx.conversationWorkspaces.capture(),
      )
      const connect = workspace.connectPreview?.bind(workspace)
      if (connect === undefined) return Promise.resolve(new Response('This conversation has no development VM', { status: 409 }))
      const location = previews.issue(() => connect(port))
      return Promise.resolve(new Response(null, { status: 303, headers: { Location: location, 'Cache-Control': 'no-store', 'Referrer-Policy': 'no-referrer' } }))
    },
