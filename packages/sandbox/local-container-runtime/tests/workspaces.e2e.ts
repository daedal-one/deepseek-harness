  ...
  it('isolates two conversations, commits residual agent writes, returns branches, and resumes the saved repository', async () => {
    ...
    try {
      const modules = new Map<string, unknown>([
        ...
        ['system-prompt', SystemPrompt], ['tools', Tools], ['loop', AgentLoop], ['container', Runtime], ['container-fs', ContainerFs], ['container-subprocess', ContainerSubprocess], ['workspaces', Workspaces], ['instructions', Instructions], ['fs-tools', FsTools],
      ])
      const entries = [
        ...
        { name: 'workspaces', config: { ...limits, poolPaths, slotBytes: 67108864, slotInodes: 20000, recoveryRoot: join(root, 'recovery'), provenanceRoot: join(root, 'provenance'), maxOutputBytes: 8388608, settleTimeoutMs: 10000, retryDelayMs: 1000, messageProvider: 'mock-metadata', messageModel: 'cheap', messageInputBytes: 4096, messageOutputTokens: 64, messageTimeoutMs: 10000 } },
