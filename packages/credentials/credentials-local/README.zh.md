...
写入是对已解析文档打补丁而不是重建，因此注释与所有未触及条目的排版都会保留。直接位于某条目上方的注释属于该条目的注解，会随它一起删除。每次 `set`、`unset` 与 `modify` 都先在 [`dsh-atomic-write`](../../util/atomic-write/README.md) 的跨进程写锁下重读文档；`modify` 在异步回调计算下一个值的整个过程持续持有该锁。提供方会发布此前未观察到的一切，再在仅属主可访问（`0700`）的目录下以 `0600` 权限原子提交，因此并发写入者、刷新 token 轮换，或落在 watcher 防抖窗口内的外部编辑会被并入，而不是被覆盖。磁盘上已经无法解析的文档会让写入失败，而不是覆盖提供方读不懂的内容。
...
- **同一引用的 `set` 调用以后写胜出**——`modify` 会串行执行根据当前值推导替换值的操作，但互相独立的 `set` 调用仍以较后的写入为准；没有修订检查。
