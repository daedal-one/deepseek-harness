...
describe('renderToolsSdkPy', () => {
  ...
  it('keeps a non-ASCII field name as a TypedDict field and derives its class name from it', () => {
    // `path` satisfies `xid_start xid_continue*`, so CPython accepts it as an
    ...
    const tool: ToolSdkSchema = {
      ...
      parameters: {
        ...
        properties: {
          path: { type: 'string' },
          opts: { type: 'object', additionalProperties: false, properties: { depth: { type: 'number' } } },
        },
        required: ['path'],
      },
      output: { type: 'string' },
    ...
    expect(text).toContain('    depth: NotRequired[float]')
    expect(text).not.toContain('dict[str, Any]')
  })
  ...
  it('subscripts a tool name that NFKC-normalizes to something else, while declaring a plain Unicode one', () => {
    // Same split at the tool-name site: `path` becomes an `async def`, the
    ...
    const text = renderToolsSdkPy([of('café'), of('ﬁnd')])
    expect(text).toContain('async def café(self, args: CaféArgs) -> str:')
