...
The `agent/turn-starting` waterfall finishes recovery before a durable turn opens. After appending `turn/end`, the loop awaits `agent/turn-settled` before admitting another turn. Listeners flush their own durable records; synchronization failures do not rewrite the model outcome.
