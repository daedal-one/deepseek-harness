...
| `provider` | `openrouter` | 写入子进程 `initialize` 的提供方路由。 |
| `model` | `deepseek/deepseek-v4-flash-0731:nitro` | 写入子进程 `initialize` 的模型。 |
| `maxTokens` | 适配器／提供方路由默认值 | 写入子进程 `initialize` 的单次请求输出 token 上限；对子运行时的根 agent 及其进程内后代生效。 |
| `env` | `{}` | 在凭据擦除后的父环境之上叠加的显式子环境（例如子进程自己的 `OPENROUTER_API_KEY`，或 `DSH_CORDIS_CONFIG`）。 |
