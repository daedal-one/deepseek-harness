...
Optional `resultValidation` selects a named validator registered on `ctx.subagents` and supplies its config-owned role. It runs only for completed one-shot results. Structured warnings accompany the canonical foreground result and are appended to rendered foreground/background text; they never replace the child's output. A missing validator rejects before child startup, while a validator failure produces the stable `validator-failed` warning.
...
| `resultValidation` | Optional `{validator, role}` policy for completed one-shot results. Both values come from plugin config, never model arguments. |
| `principal` | Optional config-owned authorization principal persisted in the local child's descriptor; never exposed as a model argument. |
