...
describe('dsh-tool-subagent', () => {
  ...
  it('registers a labeled Agent role and applies its current selection to future starts', async () => {
    ...
    adapter.resolveModel = async (provider, model) => ({
      ...
      reasoning: { efforts: [{ id: ReasoningEffortId('xhigh'), name: 'Extra high' }] },
    })
    ...
    ctx.subagents.registerProvider({
      name: 'capture',
      capabilities: { agentOptions: true, outputSchema: false, depthLimit: false, toolFilter: false, persona: false, principal: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
    ...
    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('configured-role'),
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture2',
      capabilities: { principal: false, agentOptions: false, outputSchema: false, depthLimit: true, toolFilter: true, persona: true },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture3',
      capabilities: { principal: false, agentOptions: false, outputSchema: false, depthLimit: false, toolFilter: true, persona: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('rejects startup when the provider changes during asynchronous route preflight', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'mock',
      capabilities: { principal: false, agentOptions: true, outputSchema: false, depthLimit: false, toolFilter: false, persona: false },
      inheritsParentContext: false,
...
describe('dsh-tool-subagent continuable background mode', () => {
  ...
  it('isolates a cancelled continuable preparation from a concurrent sibling', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'gated',
      capabilities: { principal: false, agentOptions: false, outputSchema: true, depthLimit: true, toolFilter: true, persona: true },
      inheritsParentContext: false,
