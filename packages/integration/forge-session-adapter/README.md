...
- Checkpoint and close evidence require `forge-intellect-action-tools/v2`, matching the pinned native Intellect executable. Set `FORGE_TEST_ACTION_MCP` to that executable when running the composition test to exercise native checkpoints and retained state across adapter restart.
