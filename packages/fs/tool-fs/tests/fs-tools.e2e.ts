...
import { E2E_MODEL, E2E_PROVIDER, fsHarness, waitForIdle } from './harness.ts'
...
describe.skipIf(!process.env.OPENROUTER_API_KEY)('fs tools with-key smoke', () => {
  it('creates, reads, then edits a file — verified on disk', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('fs-e2e'), { provider: E2E_PROVIDER, model: E2E_MODEL })
