...
Every response uses the webserver's complete-response negotiation. Index and SPA-fallback HTML is `no-store`; Vite's `/assets/` files are content-addressed and ship `public, max-age=31536000, immutable`; other static paths revalidate with `no-cache`. GET and HEAD select the same Brotli, gzip, or identity representation, while HEAD omits its body.
