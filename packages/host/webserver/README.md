...
Set `compression: 'brotli'` for Brotli with gzip fallback, or `compression: 'gzip'` for gzip alone to wrap eligible socket-backed responses without changing route APIs. The client must accept the selected encoding and the media type must be compressible; known response lengths below `compressionThresholdBytes` remain uncompressed, while unknown-length streams are eligible immediately. Existing encodings, `Cache-Control: no-transform`, range responses, SSE, ZIP, and the packaged `.gz` Worker image remain unchanged. The shipped Web bundle uses Brotli quality 9, gzip level 6, and a 1024-byte threshold; other compositions default to no compression.
