...
The package keeps strict reflection in the compiler: decorator initializers retain minimal markers in a versioned descriptor on the Service prototype. The descriptor uses a stable string property name, so another installed copy of the protocol package can read the same markers. Full parameter, result, lookup, and schema reflection is the Typert build pipeline's job, delivered through `InvocationDescriptor`. Its optional `wireFingerprint` and `semanticRevision` carry generated-schema evidence and the [authored business revision](../generator/README.md#business-revisions); missing fields mean unverified compatibility. The [generator](../generator/README.md#wire-fingerprints) owns checksum construction. Equality alone establishes neither business semantics nor authorization.
