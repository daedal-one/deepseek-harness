...
class StubProvider implements SubagentProvider {
  readonly capabilities: SubagentCapabilities = { principal: false,
    agentOptions: true,
    outputSchema: true,
    depthLimit: true,
    toolFilter: true,
  ...
  describe('script execution over a real worker thread', () => {
    ...
    it('a child result REJECTION crosses back as a fatal AGENT_RESULT error (a broken provider is not a failed child)', async () => {
      ...
      const provider: SubagentProvider = {
        name: 'rejecting',
        capabilities: { principal: false, agentOptions: true, outputSchema: true, depthLimit: true, toolFilter: true, persona: false },
