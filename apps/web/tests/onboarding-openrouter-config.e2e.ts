...
describe.skipIf(MODE === 'record')('web e2e: first-run OpenRouter credential setup', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.error(JSON.stringify({ aria: await page.locator('body').ariaSnapshot(), warnings: tripwire.warnings, errors: tripwire.pageErrors, console: browserConsole }))
      await saveFailureShot(page, 'onboarding-boot')
      throw error
    }
  }, 120_000)
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    await openRouterRow.locator('xpath=ancestor::li').getByRole('button', { name: 'Edit' }).click()
  ...
  it('configures arbitrary OpenRouter models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-openrouter-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: 'Models' }).click()
    const openRouter = settings.getByText('OpenRouter', { exact: true }).first()
    await openRouter.waitFor({ timeout: 10_000 })
    await openRouter.locator('xpath=ancestor::li').getByRole('button', { name: 'Edit' }).click()
    ...
    await settings.getByRole('button', { name: 'Capacities 1' }).click()
