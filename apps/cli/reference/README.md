...
Process shutdown gives the plugin tree five seconds to dispose by default. The inherited process variable `DSH_SHUTDOWN_TIMEOUT_MS` accepts an integer from 1 to 2147483647 milliseconds to set this grace before profile boot; project and user `.env` files cannot change it. Server deployments must allow enough time for workspace checkpointing and container disposal, with a service-manager stop deadline longer than this grace. The first `SIGINT`/`SIGTERM` starts that graceful drain — `SIGTERM` is a supervisor's ordinary stop request and exits 0 on every surface, `SIGINT` reports 130; a second signal forces immediate exit. If one-shot normal completion is already stuck in disposal, the first `Ctrl+C` is the escalation and exits immediately instead of being swallowed.
