...
This tutorial starts an isolated [Paseo](https://github.com/getpaseo/paseo) daemon that launches the built DeepSeek Harness ACP example with the committed `text-turn` replay fixture. It proves client pairing, ACP launch, prompt transport, and committed assistant text without an API key or changes to the running DSH Web service.
...
```sh
node examples/acp-agent/paseo/prepare.mjs
...
This proof uses Paseo as the session and workspace controller for one fresh ACP process. DSH ACP does not expose session listing, loading, resume, fork, model discovery, model selection, reasoning, tool presentation, plans, titles, goals, jobs, or subagents. A durable companion must use DSH as the authority for those capabilities instead of treating this compatibility proof as the product architecture.
