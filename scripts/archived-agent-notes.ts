...
/** Validate the closed kind tree, archive headers, and the frozen English-only archive. */
export function validateArchiveArtifacts(artifacts: ReadonlyMap<string, Buffer>): string[] {
  ...
  for (const [key, content] of [...sources].sort(([left], [right]) => left.localeCompare(right))) {
    const sourcePath = `${key}.md`
    const sourceBase = basename(key)
    errors.push(...validateHeader(sourcePath, content, sourceBase))
  }
  return errors
}
