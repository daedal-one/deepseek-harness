...
8. **Authoring-language slips** — untranslated working-language fragments (a stray CJK suffix such as the "client-end" word-fragment, an untranslated design-brief label, or a `---- private ----` divider) in prose whose language is otherwise English, or the reverse in a zh counterpart. Translate or delete.
