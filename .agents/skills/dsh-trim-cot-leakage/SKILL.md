...
8. **Authoring-language slips** — untranslated working-language fragments (a stray CJK suffix such as the "client-end" word-fragment, an untranslated design-brief label, or a `---- private ----` divider) in prose whose language is otherwise English,. Translate or delete.
...
3. Fix owner-first per surface: generated catalogs → trace every consumer, fix the source JSDoc or generator template, then regenerate all derivatives; type-equivalence fences → fix the source JSDoc, then re-paste the owning page (`verify-type-equiv` pins them); model- or user-visible strings → route through [dsh-prose-standard](../dsh-prose-standard/SKILL.md) and change only with owning behavior evidence, otherwise leave unchanged and report the deferral.
