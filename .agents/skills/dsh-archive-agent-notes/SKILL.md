...
1. Move the English `foo.md` from `implemented/<kind>/` to `archived/<kind>/`; `implemented` is absent from the archive path.
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`.
3. Preserve the exact note content for manifest sealing.
...
5. Run `pnpm run verify-archived-agent-notes --write`. Its append-only mode first proves every existing seal still matches, then adds only the new note hashes. Run the normal verifier afterward.
