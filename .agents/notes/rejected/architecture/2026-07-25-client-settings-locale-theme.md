...
| Language row (item order 0) | `locale` | Selector dropdown; English is the only shipped locale |
...
└─ sidebar
   └─ sidebar.settings                   single/root
      └─ ui-settings (shell, no copy)
         ├─ settings.trigger             single/root  ui-settings-general registers
         ├─ settings.header              single/root  ui-settings-general registers
         ├─ settings.close               single/root  ui-settings-general registers
         └─ settings.section             list/root
            ├─ general (order 0)         ui-settings-general registers
            │  └─ settings.general.item  list/root
            │     ├─ language (0)        locale registers
            │     └─ appearance (10)     ui-theme registers
            └─ models (order 10)         ui-settings-models registers
```
...
Locale ships with English only; `setLocale`/`setTheme` are the only write entry points, and an unknown id fails.
...
- Light/Dark/System switch and are restored after a refresh; with the preference on system, a system color-scheme change takes effect immediately.
