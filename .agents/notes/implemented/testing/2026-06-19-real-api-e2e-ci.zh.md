...
- **`DEEPSEEK_BASE_URL` 固定**为 e2e 步骤上的 `https://api.deepseek.com`。pi-ai adapter 测试在 [adapter.e2e.ts](../../../../packages/llm/llm-pi-ai/tests/adapter.e2e.ts) 中把该 endpoint 应用于 DeepSeek catalog route，因此显式取值具有自文档性和密封性——仓库根目录的 `.env`（如果存在，`vitest.e2e.config.ts` 会加载它）无法静默地将运行重定向到其他端点。
