...
- **`DEEPSEEK_BASE_URL` pinned** to `https://api.deepseek.com` on the e2e step. The pi-ai adapter test applies that endpoint to its DeepSeek catalog route in [adapter.e2e.ts](../../../../packages/llm/llm-pi-ai/tests/adapter.e2e.ts), so the explicit value is self-documenting and hermetic — a stray repo-root `.env` (which `vitest.e2e.config.ts` loads if present) cannot silently redirect the run to another endpoint.
