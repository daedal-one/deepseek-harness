...
shell 证据和审批机制由[独立工具策略证据决策](../bug-fix/2026-08-18-independent-tool-policy-evidence.md)负责。意图与命令效果使用分离且并发的模型路由，每条路由都可以通过 `ctx.llm` 传递显式且提供方中立的推理强度。分类器请求事件将该强度与重建选择器一并保留，而不复制原始输入。分类器输出可以是裸 JSON，也可以是在封闭 schema 验证前由一个精确且覆盖完整响应的 `json` Markdown 代码围栏包裹；附加文字或畸形围栏仍然无效。封闭效果进入确定性宿主策略，第一次 ask 直接进入 `ctx.approval`。无效输出、超时、路由不可用或提供者失败仍然失败关闭。调用方取消仍是取消。Daedal 为 Gemini 3.5 Flash Lite 意图复核器使用最小推理强度，因为该端点要求启用推理，且授权延迟对用户可见。
...
已知破坏性和凭据操作无需模型延迟即可失败，部署规则保持可配置，不确定操作以失败关闭方式进入现有审计审批路径，切换复核提供者无需重写策略。经过审查的 MCP 工具使用同一裁决和审批流水线，而不信任模型声明的角色。shell 提供者通常并发运行两个辅助请求，只在证据未解决时增加次效果请求；解析只读命令不使用请求。逐路由推理设置让延迟敏感的复核器使用提供方支持的最低强度，同时各部署仍负责选择其精确模型接受的强度。持久决策和审批事件跨进程重启保留，且没有进程内重试状态。MCP URL 检查减少直接访问私有网络的风险，但提供方建立连接前仍存在 DNS 重绑定间隔。
