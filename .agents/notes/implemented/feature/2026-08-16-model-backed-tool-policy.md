...
The shell evidence and approval mechanism is owned by the [independent tool-policy evidence decision](../bug-fix/2026-08-18-independent-tool-policy-evidence.md). Intent and command effects use separate concurrent model routes, closed effects feed deterministic host policy, request events retain reconstruction selectors instead of duplicated raw inputs, and the first ask enters `ctx.approval` directly. Invalid output, timeout, unavailable routing, or provider failure remains fail-closed. Caller cancellation remains cancellation.
...
Known destructive and credential operations fail without model latency, deployment rules remain configurable, uncertain actions fail closed to the existing audited approval path, and reviewer provider swaps require no policy rewrite. Reviewed MCP tools share the same verdict and approval pipeline without trusting model-asserted roles. The shell provider normally runs two auxiliary requests concurrently and adds a secondary effect request only for unresolved evidence; parsed read-only commands use none. Durable decision and approval events survive process restart without process-local retry state. MCP URL checks reduce direct private-network access but retain a DNS rebinding interval before the provider opens its connection.
