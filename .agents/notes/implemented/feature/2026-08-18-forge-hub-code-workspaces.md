...
Repository credentials reach Git through the child process environment instead of a clone URL. Replacement requests serialize, and existing checkouts are never automatically fetched or reset because preserving active edits is more important than making the directory mirror Forgejo automatically.

The shared Web and adapter runtime image includes native Forge development tools: Spec 0.8, Forge Intellect, Rust 1.97, Bun 1.4.2, Python 3.12, uv 0.11.3, and pnpm 11.7. Tool roots remain immutable and generated caches use temporary writable storage. Keeping those tools in the pinned runtime makes the edit/test loop reproducible without importing a mutable host toolchain or granting the coding process a Docker socket.

Bun uses its official musl build because glibc main-thread stack discovery needs process metadata denied to commands. The musl build preserves the filesystem and network restrictions. Its private immutable C++ libraries and loader configuration leave the other tools on their existing glibc runtime; no process-metadata grant or inherited loader environment is required.

The Forge-only command mode exposes `forge_shell`, bound `forge_fetch` and approval-bound `forge_push_branch`. Every repository inspection, instruction read, search, edit and build command uses the same Landlock launcher with a cleared environment and mandatory socket denial; other tool dispatch and scoped replacements are denied. A persona-only immutable preset removes implicit instruction-file readers and unusable native tool schemas. The CLI respects the deployment's explicit preset root, and disabling user roots prevents writable presets from adding ambient readers. Generic Harness defaults remain unchanged.
...
The keyless Linux Loader fixture exercises source read/search/edit/commit and diff alongside synthetic private-state and parent-environment canaries, including workspace symlinks, late unsafe tools, a shadowed allowed tool, exit status and cancellation. The preset fixture verifies the exact three-tool schema and absence of implicit instruction content; provider/model execution is a separate acceptance check. On the tested Linux ABI 4 kernel, confined Rust fmt/clippy/test, Python/uv, pnpm offline installation and official musl Bun 1.4.2 evaluation, TypeScript, package scripts, builtin tests and offline installation pass with unchanged filesystem grants and socket denial. Complete Forge application builds retain their own dependency and verification requirements.
...
## Continued development from remote branches

The registered clone must receive reviewed commits made elsewhere without giving a model the Forgejo service credential or unconfined network access. `forge_fetch` uses the existing persisted Session binding and a fresh private bare repository for credentialed transport. The private repository generates a bounded pack; confined `index-pack` imports its bytes without selecting any source-configured transport helper. A prepared compare-and-swap remote-tracking ref transaction runs under the same credential-free command confinement. Ref identity, ancestry, metadata aliases and final values are checked; local development state is not an import destination. No pruning or forced divergence handling is provided, and users integrate the selected commits explicitly through the confined shell.

Automatic pull/reset during catalog synchronization was rejected because it would make a roster operation rewrite ongoing work. A general credential helper was rejected because it would expose wider transport authority. The no-argument refresh tool retains one repository destination and permits ordinary collaboration while preserving dirty files, staged bytes, local branches and HEAD. A native Git smart-HTTP fixture uses only synthetic credentials and verifies remote advances, deletion retention, divergence without partial ref updates, changed-origin rejection, source transport-helper nonexecution, shared metadata and alias refusal, cancellation and the existing private-file/network denial checks.
