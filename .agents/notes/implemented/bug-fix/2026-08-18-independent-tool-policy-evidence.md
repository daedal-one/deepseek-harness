...
Common literal inspection pipelines avoid tool-time auxiliary latency. Other commands normally make one small tool-time request after one context preparation per direct user message; missing or invalid primary effect evidence can make a secondary request. The extra token cost buys independent evidence and is bounded by fixed prompts, closed line protocols, shared in-flight preparation, and secondary-on-demand dispatch.
