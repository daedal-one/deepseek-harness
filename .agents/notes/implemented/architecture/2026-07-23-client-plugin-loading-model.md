...
Production graph arrival uses one same-origin external classic `<script src>` at the graph's `bundleUrl`; that resource concatenates every independently built registration wrapper and therefore registers the complete factory table without materializing a plugin. HMR uses one graph row's `url` to replace only that factory. The browser owns each network request and script execution; the node is removed as soon as `load` or `error` settles so boot and HMR cannot accumulate dead nodes. One script-element `error` retries the same content-addressed URL through a fresh node; concurrent callers share the whole attempt, and a second error rejects. Successful production settlement requires every graph factory to exist, while HMR settlement requires its named row; registration still does not run the factory, so the side-effect boundary remains first materialization.
...
**Phase one — the module face.** The shell builds the module system over the graph, then loads `bundleUrl` once. This registers every factory before entry creation and verifies the whole table; one missing registration or a twice-failed script arrival fails boot loudly. The `immediately` declaration remains graph metadata, but production registration is graph-wide because every required plugin contributes to the one application settlement point.
