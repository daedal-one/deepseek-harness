...
Exact-detail reads belong to the current Session history generation. Teardown clears their coalescing entries before aborting and joining each request; a stale completion cannot remove a replacement request for the same sequence. Active failures reject so the caller can offer explicit retry without discarding the compact event.
