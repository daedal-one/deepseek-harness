...
分区拥有方贡献 `configurable` 标签页，由它声明既有的嵌套 `settings.plugin.item` 列表。配置卡片原有的命名空间绑定、草稿状态、校验与写入均保持不变。`@deepseek-ai/dsh-client-ui-settings-plugin-inventory` 向 `settings.plugins.tab` 贡献 `all` 标签页；它拥有 Host Loader 投影、生成的 Remote 命名空间、包元数据卡片与本地搜索。已停用的清单条目会在摘要和详情中省略重复的“未挂载”运行状态，已启用条目显示其 Cordis 阶段。
