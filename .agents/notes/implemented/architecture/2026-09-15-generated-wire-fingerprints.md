...
Equality is conservative generated-schema evidence, not semantic subtyping or proof of business behavior, permission, Session format or event vocabulary. Declaration structure and property/union order can cause conservative mismatches. Client admission, operation-time enforcement and an explicit business-semantics version policy remain required for native release. This decision extends the schema-evidence gap in [capability discovery](2026-09-15-host-capability-discovery.md); its authorization and availability reasoning remains active.
