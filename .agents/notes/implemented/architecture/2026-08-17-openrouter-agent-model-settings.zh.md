...
**pi-ai 是唯一随产品交付的对话模型 adapter，OpenRouter 是固定的部署 route。**Base、Headless、Web、ACP、SDK 和 Python runtime 组合都通过 `llm-pi-ai` 注册 OpenRouter；专用 DeepSeek LLM 包和 route 不存在。DeepSeek web search 仍是独立的搜索 provider，因为它实现 Web capability，而不是对话模型传输。
