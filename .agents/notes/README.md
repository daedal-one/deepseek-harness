...
An implemented Agent Note that is fully superseded may be consolidated into the current owning note and deleted. Before deletion, the owner must preserve every unique rationale, alternative, consequence, required verification, and named coverage gap; repair every inbound link; and preserve the current owning note in the same change. Partial supersession does not qualify: keep both notes cross-linked and update every fact that remains current. Consolidation must not rewrite the old file into its opposite or rely on git history as the only copy of rationale.
...
Active notes are English Markdown files. The frozen archive seals each English artifact in its manifest; translation files and pairing sidecars are outside this fork's maintained documentation corpus.
