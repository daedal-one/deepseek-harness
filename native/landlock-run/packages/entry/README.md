...
For offline confidential commands, require `probeConfidential()` and pass `confidential: true` to `grantArgs()`. Version 0.1.2 requires Landlock ABI 3 filesystem rights and seccomp denial of new network endpoints, descriptor stealing and cross-process memory access. Supply only pipes or ordinary files, never inherited network sockets; `socketpair` remains available for descendant IPC. Existing probe and grant behavior is unchanged when this option is omitted.
