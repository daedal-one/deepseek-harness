...
Authentication registration belongs to the provider adapter and is independent of whether the route is active. Status and operation snapshots contain only user-presentable state; the authenticator owns durable credentials and refresh.
...
Source: [`packages/llm/llm/src/index.ts:300`](../../packages/llm/llm/src/index.ts)
