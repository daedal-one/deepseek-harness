...
Principal 是部署权限事实，而不是角色标签。委派 Consumer 从配置中选择它；服务将其持久化到 descriptor version 3，并在校验子 Agent 的工具过滤器前安装已注册的子作用域能力。普通子 Agent 与根 Agent 不具有 principal。命名结果校验器是 `ctx.subagents` 上的第二个注册表：选择启用的委派 Consumer 会在子 Agent 停稳后调用它，并在不改变子 Agent 持久输出的前提下接收结构化警告。详见[受信任 principal Agent Note](../../.agents/notes/implemented/feature/2026-08-16-trusted-subagent-principals-and-results.md)。
