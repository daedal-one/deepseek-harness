...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-agent-plane`](../packages/bundle/agent-plane/README.md) then moves model-facing Consumers behind the per-session preset roster for every preset-aware surface. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a preset-aware one-shot runner with no server at all.
...
| Goal | Mechanism |
|---|---|
| Add or authenticate a model provider | register its adapter and optional authenticator on `ctx.llm` |
