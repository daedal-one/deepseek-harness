...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。随后，[`dsh-agent-plane`](../packages/bundle/agent-plane/README.md) 会在所有支持预设的界面中，将模型可见的 Consumer 移到按 Session 选择的预设名册之后。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.md) 增加支持预设的一次性运行器，且完全不带服务器。
...
| 目标 | 机制 |
|---|---|
| 添加模型提供方或为其认证 | 在 `ctx.llm` 上注册其适配器与可选认证器 |
